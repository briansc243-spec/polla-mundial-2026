    ...
    if (now >= SPECIAL_DEADLINE) {
        ...
        <details style="margin-bottom:10px;border:1px solid rgba(255,215,0,0.3);border-radius:12px;overflow:hidden;">
            <summary style="padding:13px 18px;cursor:pointer;list-style:none;display:flex;align-items:center;gap:8px;background:rgba(255,215,0,0.05);user-select:none;">
                ...
                <button onclick="event.stopPropagation();shareSpecialsWA()"
                    style="flex-shrink:0;background:#25D366;border:none;color:#fff;padding:5px 10px;border-radius:8px;cursor:pointer;font-size:0.78rem;font-weight:700;display:flex;align-items:center;gap:4px;">
    ...
    container.innerHTML = specialsHtml + eligible.map(match => {
        ...
        return `<details style="margin-bottom:10px;border:1px solid rgba(255,255,255,0.1);border-radius:12px;overflow:hidden;">
            <summary style="padding:13px 18px;cursor:pointer;list-style:none;display:flex;align-items:center;gap:8px;background:rgba(255,255,255,0.03);user-select:none;">
                ...
                <button onclick="event.stopPropagation();shareMatchPicksWA(${match.id})"
                    style="flex-shrink:0;background:#25D366;border:none;color:#fff;padding:5px 10px;border-radius:8px;cursor:pointer;font-size:0.78rem;font-weight:700;display:flex;align-items:center;gap:4px;">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                    Compartir
                </button>
...
function shareMatchPicksWA(matchId) {
    const match = matches.find(m => m.id === matchId);
    if (!match) return;
    const result = results.find(r => r.matchId === matchId);

    let lines = [];
    lines.push(`⚽ *Polla Mundial · ProntoPaga*`);
    lines.push(`${match.team1} vs ${match.team2}`);
    lines.push(result ? `✅ Resultado: *${result.score1}-${result.score2}*` : `🕐 En juego`);
    lines.push('');
    lines.push(`📊 Pronósticos:`);

    participants.forEach(p => {
        const pred = p.predictions.find(pr => pr.matchId === matchId);
        if (!pred) { lines.push(`• ${p.name} → sin pick`); return; }
        const score = `${pred.score1}-${pred.score2}`;
        let label = '';
        if (result) {
            if (pred.score1 === result.score1 && pred.score2 === result.score2) label = ' ✅ Exacto (+3pts)';
            else if (Math.sign(pred.score1 - pred.score2) === Math.sign(result.score1 - result.score2)) label = ' ↗ Tendencia (+1pt)';
            else label = ' ✗ Fallido';
        }
        lines.push(`• ${p.name} → ${score}${label}`);
    });
...
function shareSpecialsWA() {
    let lines = [];
    lines.push(`⭐ *Polla Mundial · Predicciones Especiales*`);
    lines.push(`ProntoPaga 2026`);
    lines.push('');
    ...
    participants.forEach(p => {
        ...
        const c  = sp.champion   || '—';
        const r  = sp.runnerUp   || '—';
        const t  = sp.thirdPlace || '—';
        const g  = sp.topScorer  || '—';
        lines.push(`• *${p.name}*`);
        lines.push(`  🥇 ${c}  🥈 ${r}  🥉 ${t}  ⚽ ${g}`);
    });
